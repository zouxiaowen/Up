...
public class DoubleWaveView extends View {
    ...
    private static  int TRANSLATE_X_SPEED_Three ;    // 第二条水波的移动速度 单位dp
    private int mXOffsetSpeedOne;//单位 PX
    private int mXOffsetSpeedTwo;//单位 PX
    private int mXOffsetSpeedThree;//单位 PX
    ...
    private int mXThreeOffset;//第三条波浪的X轴 偏移量
    ...
    public DoubleWaveView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.DoubleWaveView_peakValue) {//振幅默认是30dp
                ...
                TRANSLATE_X_SPEED_ONE = a.getInteger(attr, 9);//默认是7

            } else if (attr == R.styleable.DoubleWaveView_speedTwo) {
                TRANSLATE_X_SPEED_TWO = a.getInteger(attr, 7);//默认是5

            } else if (attr == R.styleable.DoubleWaveView_speedThree) {
                TRANSLATE_X_SPEED_Three = a.getInteger(attr, 5);//默认是3
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i=0,j=0,k=0,xx=0;i<mTotalWidth;i++){
            ...
            if(i+mXThreeOffset<mTotalWidth){//第二条波纹图形绘制
                canvas.drawLine(i,mTotalHeight-mYPositions[mXThreeOffset+i]-WaveHeight,i,mTotalHeight,mWavePaint);
            }else {//大于周期值，则设置为xx(与相位相关，已移动的X距离)
                canvas.drawLine(i,mTotalHeight-mYPositions[xx]-WaveHeight,i,mTotalHeight,mWavePaint);
                xx++;
            }
